...
class DataBase:
    ...
    def _fetch_query_results(self, query: str, params: Optional[Dict[str, Any]] = None) -> List[tuple]:
    ...
    def _get_entity_id(self, entity_type: str, field: str, value: str) -> int:
        ...
        with self.pool.connect() as conn:
            ...
            if result:
                return result[0]
            raise ValueError(f"{entity_type[:-1].capitalize()} with {field} {value} not found")
    ...
    def get_readings(self, facility_name: str, meter_type: Optional[str] = None) -> List[APIReading]:
        ...
        base_query = """
        SELECT value, reading_date, serial_number, email
        FROM readings
    ...
    def delete_reading(self, user_email: str, meter_serial_number: str) -> None:
        ...
        meter_id = self._get_meter_id(meter_serial_number)
        
        # Fixed query - PostgreSQL doesn't support ORDER BY in DELETE without subquery
        query = """
        DELETE FROM readings
        WHERE reading_id = (
            SELECT reading_id FROM readings
            WHERE user_id = :user_id AND meter_id = :meter_id
    ...
    def update_user(self, user_data: APIUser) -> None:
